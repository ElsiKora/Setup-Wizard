...
describe("CiModuleService", () => {
	...
	beforeEach(() => {
		...
		ciModuleService = new CiModuleService(cliInterfaceServiceMock, fileSystemServiceMock, configServiceMock);
	});
	...
	describe("handleExistingSetup", () => {
		it("should return true when no existing files are found", async () => {
			...
			const spyFindExistingCiFiles = vi.spyOn(ciModuleService as any, "findExistingCiFiles").mockResolvedValue([]);
		...
		it("should ask for confirmation when existing files are found", async () => {
			...
			vi.spyOn(ciModuleService as any, "findExistingCiFiles").mockResolvedValue([".github/workflows/release.yml"]);
			...
			expect(cliInterfaceServiceMock.confirm).toHaveBeenCalledWith("Do you want to continue? This might overwrite existing files.", false);
		});

		it("should handle errors and return false", async () => {
			// Mock private method findExistingCiFiles to throw an error
			vi.spyOn(ciModuleService as any, "findExistingCiFiles").mockRejectedValue(new Error("Test error"));

			const result = await ciModuleService.handleExistingSetup();

			expect(result).toBe(false);
			expect(cliInterfaceServiceMock.handleError).toHaveBeenCalledWith("Failed to check existing CI setup", expect.any(Error));
		});
	});
	...
	describe("shouldInstall", () => {
		it("should return true when user confirms installation", async () => {
			...
			expect(result).toBe(true);
			expect(cliInterfaceServiceMock.confirm).toHaveBeenCalledWith("Would you like to set up CI workflows?", false);
		});
		...
		it("should use saved configuration if available", async () => {
			...
			expect(cliInterfaceServiceMock.confirm).toHaveBeenCalledWith("Would you like to set up CI workflows?", true);
		});
		...
		it("should handle errors and return false", async () => {
			...
			expect(result).toBe(false);
			expect(cliInterfaceServiceMock.handleError).toHaveBeenCalledWith("Failed to get user confirmation", expect.any(Error));
		});
	});
	...
	describe("install", () => {
		beforeEach(() => {
			...
			vi.spyOn(ciModuleService as any, "determineModuleType").mockResolvedValue(ECiModuleType.UNIVERSAL);

			vi.spyOn(ciModuleService as any, "selectProvider").mockResolvedValue(ECiProvider.GITHUB);

			vi.spyOn(ciModuleService as any, "selectCompatibleModules").mockResolvedValue([ECiModule.DEPENDABOT]);

			vi.spyOn(ciModuleService as any, "handleExistingSetup").mockResolvedValue(true);

			vi.spyOn(ciModuleService as any, "setupSelectedModules").mockResolvedValue({ [ECiModule.DEPENDABOT]: { devBranchName: "dev" } });
		...
		it("should return wasInstalled: false when no modules are selected", async () => {
			vi.spyOn(ciModuleService as any, "selectCompatibleModules").mockResolvedValue([]);
		...
		it("should return wasInstalled: false when user cancels due to existing setup", async () => {
			vi.spyOn(ciModuleService as any, "handleExistingSetup").mockResolvedValue(false);
		...
		it("should handle and re-throw errors during installation", async () => {
			...
			vi.spyOn(ciModuleService as any, "setupSelectedModules").mockRejectedValue(error);

			await expect(ciModuleService.install()).rejects.toThrow("Test error");

			expect(cliInterfaceServiceMock.handleError).toHaveBeenCalledWith("Failed to complete CI setup", error);
		});
	});
	...
	describe("Private methods", () => {
		describe("determineModuleType", () => {
			it("should return NPM_ONLY when user confirms package is for NPM", async () => {
				...
				expect(cliInterfaceServiceMock.confirm).toHaveBeenCalledWith("Is this package going to be published to NPM?", false);
			});
			...
			it("should use saved value as default if provided", async () => {
				...
				expect(result).toBe(ECiModuleType.NPM_ONLY);
				expect(cliInterfaceServiceMock.confirm).toHaveBeenCalledWith("Is this package going to be published to NPM?", true);
			});
		});
		...
		describe("selectProvider", () => {
			...
			it("should use saved provider as default if provided", async () => {
				...
				expect(result).toBe(ECiProvider.GITHUB);
				expect(cliInterfaceServiceMock.select).toHaveBeenCalledWith("Select CI provider:", expect.any(Array), ECiProvider.GITHUB);
			});
		});
		...
		describe("selectCompatibleModules", () => {
			it("should filter modules based on module type and prompt user to select", async () => {
				...
				const result = await (ciModuleService as any).selectCompatibleModules(ECiModuleType.UNIVERSAL, []);
		...
		describe("setupModule", () => {
			...
			it("should create directory and write file for the module", async () => {
				...
				const result = await (ciModuleService as any).setupModule(ECiModule.DEPENDABOT, { devBranchName: "dev" });
				...
				expect(fileSystemServiceMock.createDirectory).toHaveBeenCalledWith(".github", {
					isRecursive: true,
				});

				expect(fileSystemServiceMock.writeFile).toHaveBeenCalledWith(".github/dependabot.yml", expect.stringContaining("version: 2"));
			});
		...
		describe("collectModuleProperties", () => {
			it("should collect properties for DEPENDABOT module", async () => {
				...
				const result = await (ciModuleService as any).collectModuleProperties(ECiModule.DEPENDABOT, {});

				expect(result).toEqual({
					devBranchName: "feature",
				});

				expect(cliInterfaceServiceMock.text).toHaveBeenCalledWith("Enter the target branch for Dependabot updates:", "dev", "dev");
			});

			it("should use saved properties for DEPENDABOT module if available", async () => {
				cliInterfaceServiceMock.text.mockResolvedValue("feature");

				const result = await (ciModuleService as any).collectModuleProperties(ECiModule.DEPENDABOT, { devBranchName: "staging" });

				expect(result).toEqual({
					devBranchName: "feature",
				});

				expect(cliInterfaceServiceMock.text).toHaveBeenCalledWith("Enter the target branch for Dependabot updates:", "dev", "staging");
			});
			...
			it("should collect properties for RELEASE module", async () => {
				...
				const result = await (ciModuleService as any).collectModuleProperties(ECiModule.RELEASE, {});
			...
			it("should collect properties for RELEASE module with prerelease disabled", async () => {
				...
				expect(cliInterfaceServiceMock.text).not.toHaveBeenCalledWith(expect.stringContaining("pre-release branch"), expect.any(String), expect.any(String));
			});
			...
			it("should use saved properties for RELEASE module if available", async () => {
				...
				expect(cliInterfaceServiceMock.text).toHaveBeenCalledWith(expect.any(String), expect.any(String), "production");

				// Verify the preReleaseBranch question was asked with saved value
				expect(cliInterfaceServiceMock.text).toHaveBeenCalledWith(expect.any(String), expect.any(String), "develop");
			});
			...
			it("should handle RELEASE_NPM module the same as RELEASE module", async () => {
				...
				const result = await (ciModuleService as any).collectModuleProperties(ECiModule.RELEASE_NPM, {});
			...
			it("should use semantic-release config if available for RELEASE module", async () => {
				...
				expect(cliInterfaceServiceMock.info).toHaveBeenCalledWith("Found semantic-release configuration. Using its values as defaults for release CI setup.");
			});
			...
			it("should return empty object for unknown modules", async () => {
				...
				const result = await (ciModuleService as any).collectModuleProperties(ECiModule.SNYK, {});
		...
		describe("displaySetupSummary", () => {
			it("should display successful and failed configurations", () => {
				(ciModuleService as any).displaySetupSummary([{ module: ECiModule.DEPENDABOT }], [{ module: ECiModule.SNYK, error: new Error("API key required") }]);

				expect(cliInterfaceServiceMock.note).toHaveBeenCalledWith("CI Setup Summary", expect.stringContaining("Successfully created configurations:"));
		...
		describe("setupSelectedModules", () => {
			...
			it("should handle setup failures for some modules", async () => {
				...
				expect((ciModuleService as any).displaySetupSummary).toHaveBeenCalledWith([{ isSuccess: true, module: ECiModule.DEPENDABOT }], [{ isSuccess: false, module: ECiModule.SNYK, error: expect.any(Error) }]);
			});
			...
			it("should throw error if provider is not selected", async () => {
				...
				await expect((ciModuleService as any).setupSelectedModules({})).rejects.toThrow("Provider not selected");
			...
			it("should handle errors during setup process", async () => {
				...
				await expect((ciModuleService as any).setupSelectedModules({})).rejects.toThrow("Setup error");
			...
			it("should use saved module properties if available", async () => {
				...
				expect((ciModuleService as any).collectModuleProperties).toHaveBeenCalledWith(ECiModule.DEPENDABOT, { devBranchName: "main" });
			});
