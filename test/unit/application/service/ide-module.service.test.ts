...
describe('IdeModuleService', () => {
  ...
  beforeEach(() => {
    ...
    ideModuleService = new IdeModuleService(
      cliInterfaceServiceMock,
      fileSystemServiceMock,
      configServiceMock
    );
  ...
  describe('handleExistingSetup', () => {
    it('should return true when no existing files are found', async () => {
      ...
      vi.spyOn(ideModuleService as any, 'findExistingConfigFiles')
        .mockResolvedValue([]);
    ...
    it('should ask for confirmation when existing files are found', async () => {
      ...
      vi.spyOn(ideModuleService as any, 'findExistingConfigFiles')
        .mockResolvedValue(['.vscode/settings.json']);
      ...
      expect(result).toBe(true);
      expect(cliInterfaceServiceMock.warn).toHaveBeenCalledWith(
        expect.stringContaining('Found existing IDE configuration files')
      );
  ...
  describe('shouldInstall', () => {
    it('should return true when user confirms installation', async () => {
      ...
      expect(result).toBe(true);
      expect(cliInterfaceServiceMock.confirm).toHaveBeenCalledWith(
        'Would you like to set up ESLint and Prettier configurations for your code editors?',
        false
      );
    });
  ...
  describe('install', () => {
    beforeEach(() => {
      ...
      vi.spyOn(ideModuleService as any, 'selectIdes')
        .mockResolvedValue([EIde.VS_CODE]);
      
      vi.spyOn(ideModuleService as any, 'handleExistingSetup')
        .mockResolvedValue(true);
      
      vi.spyOn(ideModuleService as any, 'setupSelectedIdes')
    ...
    it('should call selectIdes with empty array when config.ides is undefined', async () => {
      configServiceMock.getModuleConfig.mockResolvedValue({ /* no ides property */ });
    ...
    it('should return wasInstalled: false when no IDEs are selected', async () => {
      ...
      expect(result).toEqual({ wasInstalled: false });
      expect(cliInterfaceServiceMock.warn).toHaveBeenCalledWith('No IDEs selected.');
    });
  ...
  describe('Private methods', () => {
    ...
    describe('setupSelectedIdes', () => {
      beforeEach(() => {
        ...
        vi.spyOn(ideModuleService as any, 'setupIde')
          .mockImplementation((ide: EIde) => {
            if (ide === EIde.VS_CODE) {
      ...
      it('should set up all selected IDEs and display summary', async () => {
        ...
        // Should use spinner
        expect(cliInterfaceServiceMock.startSpinner).toHaveBeenCalledWith(
          'Setting up IDE configurations...'
        );
        expect(cliInterfaceServiceMock.stopSpinner).toHaveBeenCalledWith(
          'IDE configuration completed successfully!'
        );
    ...
    describe('findExistingConfigFiles', () => {
      ...
      it('should return empty array when no files exist', async () => {
        ...
        expect(result).toEqual([]);
        expect(fileSystemServiceMock.isPathExists).toHaveBeenCalledWith(
          IDE_CONFIG[EIde.VS_CODE].content[0].filePath
        );
      });
    ...
    describe('displaySetupSummary', () => {
      ...
      it('should use "Unknown error" when error has no message property', () => {
        ...
        const summaryMessage = cliInterfaceServiceMock.note.mock.calls[0][1];
        expect(summaryMessage).toContain('✗ IntelliJ IDEA - Unknown error');
      });

      it('should only display successful configurations when no failures', () => {
        const successful = [
          { ide: EIde.VS_CODE },
          { ide: EIde.INTELLIJ_IDEA }
        ];
