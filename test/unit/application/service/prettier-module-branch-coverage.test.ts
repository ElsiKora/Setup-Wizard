...
import { PRETTIER_CONFIG_IGNORE_PATHS } from '../../../../src/application/constant/prettier-config-ignore-paths.constant';
  ...
  describe("createConfigs", () => {
    ...
    it("should create both config and ignore files", async () => {
      ...
      expect(mockFileSystemService.writeFile).toHaveBeenCalledWith(
        ...
        PRETTIER_CONFIG_IGNORE_PATHS.join("\n"),
        "utf8"
      );
    });
    ...
    it("should handle errors when writing files", async () => {
      ...
      mockFileSystemService.writeFile
        .mockRejectedValueOnce(new Error("Test error writing config"))
