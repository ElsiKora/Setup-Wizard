...
import { PRETTIER_CONFIG_FILE_NAME } from '../../../../src/application/constant/prettier-config-file-name.config';
import { PRETTIER_CONFIG_IGNORE_FILE_NAME } from '../../../../src/application/constant/prettier-config-ignore-file-name.constant';
import { PRETTIER_CONFIG_CORE_DEPENDENCIES } from '../../../../src/application/constant/prettier-config-core-dependencies.constant';
...
describe('PrettierModuleService', () => {
  ...
  beforeEach(() => {
    ...
    prettierService = new PrettierModuleService(
  ...
  describe('shouldInstall', () => {
    it('should return true when user confirms installation', async () => {
      ...
      expect(mockCliInterfaceService.confirm).toHaveBeenCalledWith(
        'Do you want to set up Prettier for your project?',
        true
      );
    });
  ...
  describe('private methods', () => {
    it('setupPrettier should install dependencies, create configs and setup scripts', async () => {
      ...
      expect(mockPackageJsonService.installPackages).toHaveBeenCalledWith(
        PRETTIER_CONFIG_CORE_DEPENDENCIES,
        'latest',
    ...
    it('setupPrettier should handle errors properly', async () => {
      ...
      expect(mockCliInterfaceService.startSpinner).toHaveBeenCalled();
      expect(mockCliInterfaceService.stopSpinner).toHaveBeenCalledWith('Failed to setup Prettier configuration');
    });
    ...
    it('displaySetupSummary should show the proper summary information', () => {
      ...
      expect(mockCliInterfaceService.note).toHaveBeenCalledWith(
        'Prettier Setup',
        expect.stringContaining('Prettier configuration has been created.')
      );
      expect(mockCliInterfaceService.note).toHaveBeenCalledWith(
        'Prettier Setup',
        expect.stringContaining('npm run format')
      );
      expect(mockCliInterfaceService.note).toHaveBeenCalledWith(
        'Prettier Setup',
        expect.stringContaining(PRETTIER_CONFIG_FILE_NAME)
      );
      expect(mockCliInterfaceService.note).toHaveBeenCalledWith(
        'Prettier Setup',
        expect.stringContaining(PRETTIER_CONFIG_IGNORE_FILE_NAME)
      );
    });
  });
});