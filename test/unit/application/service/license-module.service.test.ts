...
describe("LicenseModuleService", () => {
  ...
  describe("shouldInstall", () => {
    it("should return true when user confirms installation", async () => {
      ...
      expect(mockCliInterfaceService.confirm).toHaveBeenCalledWith(
        "Do you want to generate LICENSE for your project?", 
        true
      );
    });
  ...
  describe("handleExistingSetup", () => {
    ...
    it("should ask to replace when existing license file is found and user confirms", async () => {
      ...
      expect(result).toBe(true);
      expect(mockCliInterfaceService.confirm).toHaveBeenCalledWith(
        "An existing license file was found (LICENSE). Would you like to replace it?"
      );
      expect(mockFileSystemService.deleteFile).toHaveBeenCalledWith("LICENSE");
      expect(mockCliInterfaceService.success).toHaveBeenCalledWith("Deleted existing license file.");
    });
    ...
    it("should return false when user declines to replace existing license", async () => {
      ...
      expect(result).toBe(false);
      expect(mockCliInterfaceService.warn).toHaveBeenCalledWith("Keeping existing license file.");
    });
    ...
    it("should handle errors when deleting existing license file", async () => {
      ...
      expect(mockCliInterfaceService.handleError).toHaveBeenCalledWith(
        "Failed to delete existing license file", 
        expect.any(Error)
      );
    });
    ...
    it("should handle errors when checking for existing license", async () => {
      ...
      expect(result).toBe(false);
      expect(mockCliInterfaceService.handleError).toHaveBeenCalledWith(
        "Failed to check existing license setup", 
        expect.any(Error)
      );
    });
  });
  ...
  describe("install", () => {
    ...
    it("should handle errors during installation", async () => {
      ...
      expect(mockCliInterfaceService.handleError).toHaveBeenCalledWith(
        "Failed to complete license installation",
        expect.any(Error)
      );
    });
  ...
  describe("createLicenseFile", () => {
    it("should create license file with author from package.json", async () => {
      ...
      expect(mockCliInterfaceService.startSpinner).toHaveBeenCalledWith("Generating license file...");
      expect(mockFileSystemService.writeFile).toHaveBeenCalledWith(
        "LICENSE",
        LICENSE_CONFIG[ELicense.MIT].template(currentYear.toString(), "Final Author")
      );
      expect(mockPackageJsonService.setProperty).toHaveBeenCalledWith("license", ELicense.MIT);
      expect(mockCliInterfaceService.stopSpinner).toHaveBeenCalledWith("License file generated");
    });
    ...
    it("should handle errors when getting author from package.json", async () => {
      ...
      expect(mockCliInterfaceService.warn).toHaveBeenCalledWith("Failed to get author from package.json, using saved or default");
      expect(mockCliInterfaceService.text).toHaveBeenCalledWith(
        "Enter the copyright holder's name:",
        "Your Name",
        "Your Name"
  ...
  describe("selectLicense", () => {
    it("should prompt user to select a license", async () => {
      ...
      expect(mockCliInterfaceService.select).toHaveBeenCalledWith(
  ...
  describe("displaySetupSummary", () => {
    it("should display successful setup summary", () => {
      ...
      expect(mockCliInterfaceService.note).toHaveBeenCalledWith(
        "License Setup Summary",
        expect.stringContaining(`- Type: ${LICENSE_CONFIG[ELicense.MIT].name}`)
      );
      expect(mockCliInterfaceService.note).toHaveBeenCalledWith(
        "License Setup Summary",
    ...
    it("should display 'Unknown error' when error has no message", () => {
      ...
      expect(mockCliInterfaceService.note).toHaveBeenCalledWith(
        "License Setup Summary",
        expect.stringContaining("✗ LICENSE - Unknown error")
      );
    });
    ...
    it("should use default author name when not provided", () => {
      ...
      expect(mockCliInterfaceService.note).toHaveBeenCalledWith(
        "License Setup Summary",
        expect.stringContaining("- Author: Your Name")
      );
    });
  });
});