...
describe("EslintModuleService Branch Coverage", () => {
	...
	describe("displaySetupSummary", () => {
		...
		it("should properly handle frameworks with no description", async () => {
			...
			(eslintService as any).detectedFrameworks = [{ name: EFramework.REACT, displayName: "React", description: "" }];
	...
	describe("selectFeatures", () => {
		it("should handle invalid saved features", async () => {
			...
			vi.spyOn(eslintService as any, "detectInstalledFeatures").mockResolvedValueOnce([EEslintFeature.JAVASCRIPT, EEslintFeature.TYPESCRIPT]);

			// User accepts detected features
			mockCliInterfaceService.confirm.mockResolvedValueOnce(true);

			// Mock user selection
			mockCliInterfaceService.groupMultiselect.mockResolvedValueOnce([EEslintFeature.JAVASCRIPT, EEslintFeature.TYPESCRIPT]);
		...
		it("should handle only one detected feature", async () => {
			...
			vi.spyOn(eslintService as any, "detectInstalledFeatures").mockResolvedValueOnce([EEslintFeature.JAVASCRIPT]);

			// Mock user selection
			mockCliInterfaceService.groupMultiselect.mockResolvedValueOnce([EEslintFeature.JAVASCRIPT]);
