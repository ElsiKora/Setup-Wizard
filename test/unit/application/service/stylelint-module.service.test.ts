...
import { STYLELINT_CONFIG_FILE_NAMES } from "../../../../src/application/constant/stylelint/file-names.constant";
import { STYLELINT_CONFIG_FILE_NAME } from "../../../../src/application/constant/stylelint/file-name.constant";
import { STYLELINT_CONFIG_IGNORE_FILE_NAME } from "../../../../src/application/constant/stylelint/ignore-file-name.constant";
import { STYLELINT_CONFIG } from "../../../../src/application/constant/stylelint/config.constant";
import { STYLELINT_CONFIG_IGNORE_PATHS } from "../../../../src/application/constant/stylelint/ignore-paths.constant";
import { STYLELINT_CONFIG_CORE_DEPENDENCIES } from "../../../../src/application/constant/stylelint/core-dependencies.constant";
...
describe("StylelintModuleService", () => {
	...
	describe("shouldInstall", () => {
		it("should return true when user confirms installation", async () => {
			...
			expect(mockCliInterfaceService.confirm).toHaveBeenCalledWith("Do you want to set up Stylelint for your project?", true);
		});
	...
	describe("handleExistingSetup", () => {
		...
		it("should ask to delete when existing files are found and user confirms", async () => {
			...
			expect(result).toBe(true);
			expect(mockCliInterfaceService.confirm).toHaveBeenCalledWith(expect.stringContaining("Existing Stylelint configuration files detected"), true);
		...
		it("should return false when user declines to delete existing files", async () => {
			...
			expect(result).toBe(false);
			expect(mockCliInterfaceService.warn).toHaveBeenCalledWith("Existing Stylelint configuration files detected. Setup aborted.");
	...
	describe("createConfigs", () => {
		it("should create stylelint configuration files", async () => {
			...
			expect(mockFileSystemService.writeFile).toHaveBeenCalledWith(STYLELINT_CONFIG_FILE_NAME, STYLELINT_CONFIG, "utf8");
			expect(mockFileSystemService.writeFile).toHaveBeenCalledWith(STYLELINT_CONFIG_IGNORE_FILE_NAME, STYLELINT_CONFIG_IGNORE_PATHS.join("\n"), "utf8");
		});
	});
	...
	describe("setupScripts", () => {
		it("should add stylelint scripts to package.json", async () => {
			...
			expect(mockPackageJsonService.addScript).toHaveBeenCalledWith("lint:style", 'stylelint "**/*.{css,scss}"');
			expect(mockPackageJsonService.addScript).toHaveBeenCalledWith("lint:style:fix", 'stylelint "**/*.{css,scss}" --fix');
		});
	});
	...
	describe("displaySetupSummary", () => {
		it("should display setup summary with correct information", () => {
			...
			expect(mockCliInterfaceService.note).toHaveBeenCalledWith("Stylelint Setup", expect.stringContaining("Stylelint configuration has been created."));
			expect(mockCliInterfaceService.note).toHaveBeenCalledWith("Stylelint Setup", expect.stringContaining("Generated scripts:"));
			expect(mockCliInterfaceService.note).toHaveBeenCalledWith("Stylelint Setup", expect.stringContaining(`- ${STYLELINT_CONFIG_FILE_NAME}`));
			expect(mockCliInterfaceService.note).toHaveBeenCalledWith("Stylelint Setup", expect.stringContaining(`- ${STYLELINT_CONFIG_IGNORE_FILE_NAME}`));
		});
	});
	...
	describe("setupStylelint", () => {
		it("should set up stylelint configuration successfully", async () => {
			...
			expect(mockPackageJsonService.installPackages).toHaveBeenCalledWith(STYLELINT_CONFIG_CORE_DEPENDENCIES, "latest", EPackageJsonDependencyType.DEV);
	...
	describe("install", () => {
		...
		it("should handle errors during installation", async () => {
			...
			expect(mockCliInterfaceService.handleError).toHaveBeenCalledWith("Failed to complete Stylelint setup", expect.any(Error));
		});
	});
});
