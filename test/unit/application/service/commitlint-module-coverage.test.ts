...
import { COMMITLINT_CONFIG_HUSKY_PRE_PUSH_SCRIPT } from '../../../../src/application/constant/commitlint-config-husky-pre-push-script.constant';
...
describe('CommitlintModuleService Full Coverage', () => {
  ...
  beforeEach(() => {
    ...
    commitlintService = new CommitlintModuleService(
  ...
  it('should find all existing configuration files', async () => {
    ...
    expect(existingFiles).not.toContain('.husky/pre-push');
    ...
    expect(mockFileSystemService.isPathExists).toHaveBeenCalledWith('.husky/pre-push');
    ...
    expect(allExistingFiles).toContain('.husky/pre-push');
  ...
  it('should display the setup summary', () => {
    ...
    expect(mockCliInterfaceService.note).toHaveBeenCalledWith(
      'Commitlint Setup',
      expect.stringContaining('Commitlint and Commitizen configuration has been created.')
    );
    expect(mockCliInterfaceService.note).toHaveBeenCalledWith(
      'Commitlint Setup',
      expect.stringContaining('npm run commit')
    );
  });
  ...
  it('should set up husky git hooks', async () => {
    ...
    expect(mockFileSystemService.writeFile).toHaveBeenCalledWith(
      '.husky/commit-msg',
      COMMITLINT_CONFIG_HUSKY_COMMIT_MSG_SCRIPT,
      'utf8'
    );
    ...
    expect(mockFileSystemService.writeFile).toHaveBeenCalledWith(
      '.husky/pre-push',
      COMMITLINT_CONFIG_HUSKY_PRE_PUSH_SCRIPT,
      'utf8'
    );
    expect(mockCommandService.execute).toHaveBeenCalledWith('chmod +x .husky/pre-push');
  });
  ...
  it('should handle all branches in the shouldInstall method', async () => {
    ...
    expect(mockCliInterfaceService.confirm).toHaveBeenCalledWith(
      'Do you want to set up Commitlint and Commitizen for your project?',
      true
    );
