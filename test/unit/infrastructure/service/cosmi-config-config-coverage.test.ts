...
describe("CosmicConfigService Coverage Tests", () => {
	...
	describe("writeFile with additional file extensions", () => {
		it("should write YML config correctly", async () => {
			...
			expect(mockFileSystemService.writeFile).toHaveBeenCalledWith("./.elsikora/setuprc.yml", `YAML:${JSON.stringify(config)}`, "utf8");
		});
		...
		it("should use default JSON format for unknown extensions", async () => {
			...
			expect(mockFileSystemService.writeFile).toHaveBeenCalledWith("config.unknown", JSON.stringify(config, null, 2), "utf8");
		});
	});
});
