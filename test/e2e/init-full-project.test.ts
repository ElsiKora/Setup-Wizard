...
vi.mock("@clack/prompts", () => {
	return {
		...
		multiselect: vi.fn((options) => {
			...
			return Promise.resolve(["eslint", "prettier", "commitlint", "lint-staged", "semantic-release"]);
		}),
		spinner: vi.fn(() => ({
			start: vi.fn(),
			stop: vi.fn(),
...
describe("Full Project Initialization E2E Test", () => {
    ...
    `;
		...
		await fs.writeFile(path.join(testUtils.testDir, "eslint.config.js"), eslintConfigContent, "utf8");
    ...
    `;
		...
		await fs.writeFile(path.join(testUtils.testDir, "commitlint.config.js"), commitlintConfigContent, "utf8");
    ...
    `;
		...
		await fs.writeFile(path.join(testUtils.testDir, "lint-staged.config.js"), lintStagedConfigContent, "utf8");
    ...
    `;
		...
		await fs.writeFile(path.join(testUtils.testDir, "release.config.js"), releaseConfigContent, "utf8");
    ...
    `;
		...
		await fs.writeFile(path.join(testUtils.testDir, ".gitignore"), gitignoreContent, "utf8");
	...
	it("should ensure all tools work together correctly", async () => {
      ...
      `;
		...
		await fs.writeFile(testFile, badCode, "utf8");
